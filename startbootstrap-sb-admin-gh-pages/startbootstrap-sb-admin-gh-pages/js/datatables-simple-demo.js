window.addEventListener("DOMContentLoaded", (event) => {
  ...
  const successCardDescription = document.getElementById("successCardDescription");
  console.log(successCardDescription)
  if (successCardDescription) {
    const randomEntry = tableData[getRandomInt(tableData.length - 1)];
    successCardDescription.innerHTML = `
      <p>Name: ${randomEntry.name}</p>
      <p>Position: ${randomEntry.position}</p>
      <p>Office: ${randomEntry.office}</p>
      <p>Age: ${randomEntry.age}</p>
      <p>Start Date: ${randomEntry.startDate}</p>
      <p>Salary: ${randomEntry.salary}</p>
    `;
  } else {
    console.error("successCardDescription not found.");
  }
});
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    document.getElementById(
      "averageSalary"
    ).textContent = `Number of Entries: ${entryCount}, Average Salary: $${averageSalary}`;
  });
